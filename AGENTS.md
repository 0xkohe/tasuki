...
## Configuration Notes
Local config resolves from `.tasuki/config.yaml`; global config uses the user config directory. Do not hardcode machine-specific paths, credentials, or provider tokens in tests or sample configs.
